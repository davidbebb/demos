...
#	x = -k_width/2+ i * 2* k_amp
	...
	if i%2:
		...
		for j in range(-1, num_ky+1):
			...
			potassium.add(Hole(V(x-k_amp, k_height/2 - j*k_wl+ball_offset*k_wl), rad=k_rad/2, z1=-kb_depth), 'Potassium')
			potassium.add(Hole(V(x+k_amp, k_height/2 - (0.5+j)*k_wl+ball_offset*k_wl), z1=-kb_depth, rad=k_rad/2), 'Potassium')
...
#	x = -li_width/2+ i * li_amp
	...
	if i%2:
		...
		for j in range(-1, num_liy):
			...
			lithium.add(Hole(V(x-li_amp, li_height/2 - j*li_wl+ball_offset*li_wl), rad=li_rad/2, z1=-lib_depth))
			lithium.add(Hole(V(x+li_amp, li_height/2 - (0.5+j)*li_wl+ball_offset*li_wl), z1=-lib_depth, rad=li_rad/2))
